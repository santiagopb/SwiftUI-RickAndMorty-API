...
class CharacterListViewModel: ObservableObject {
    ...
    private func getCharacters() {
        guard !isLastPage else {
            Log.debug(tag: CharacterListViewModel.self, message: "We are on the last page we can not ask for more")
            return
        }
        ...
        service.getCharacters(page: page,
            ...
            case .success(let dataWrapper):
                dataWrapper.results?.forEach({ character in
                    if let _character = Character.build(apiObject: character) {
                        ...
                        Log.error(tag: CharacterListViewModel.self, message: "It was not posible to build a Character from data -> \(character)")
                    }
                })
            ...
            case .failure(let error):
                Log.error(tag: CharacterListViewModel.self, message: "It was not posible to load Characters", error: error)
            }
    ...
    public func applyFilter(_ filterData: CharacterFilterData) {
        if self.filterData == filterData {
            Log.debug(tag: CharacterListViewModel.self, message: "Filter values ​​have not changed")
        } else {
